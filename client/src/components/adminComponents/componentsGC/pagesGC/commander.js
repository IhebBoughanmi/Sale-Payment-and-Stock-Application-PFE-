    ...
    .then(res => {
        if(res.status===200){
          ...
          const timer = setTimeout(() => {
            setSubmitted(false)
            props.handleCallback("MAJ acquis a cotions");
          }, 3000);
        }
        else{
          setError(true)
...
<p style={{color:'red'}}> {Modelivraisonerror!='vrai'?Modelivraisonerror:''}</p>
 ...
 <div class="input-group">
   ...
   <div class="btns-group">
     ...
     <input value="Envoyer" class="btn ml-auto " onClick={(e)=>{submit(el.mail,el.nom,el.prenom);e.preventDefault();}}  style={{background:'rgb(11, 78, 179)',color:'white',width:'150px'}}/> </>
                    )
                  })}
