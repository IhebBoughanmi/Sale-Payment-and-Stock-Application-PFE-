...
export default function Acqui(props) {
    ...
    .then(res => {
        if(res.status===200){
          ...
          const timer = setTimeout(() => {
            setSubmitted('')
            props.handleCallback("Commander");
          }, 3000);
        }
        else{
          setError(true)
...
<td> <TextField variant="standard" id="demo-helper-text-misaligned-no-helper"
 onChange={(e)=>setNbrfut(e.target.value)}  value={el.Nbrfut} fullWidth disabled /></td>
  ...
  <td> <TextField variant="standard" id="demo-helper-text-misaligned-no-helper" label="" fullWidth disabled value="99.96"/></td>
  </>
      )
    })}
         </table>
